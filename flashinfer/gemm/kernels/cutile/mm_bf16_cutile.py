...
def mm_bf16_cutile(
    ...
    Bias handling
    -------------
    The underlying alpha-beta kernel computes
    ``result = alpha * acc + beta * c_load`` in fp32 in the epilogue. We use:
      * bias is None  -> alpha=1, beta=0, ``out`` zero-initialized (NaN guard)
      * bias != None  -> alpha=1, beta=1 with bias *pre-broadcast* into ``out``;
        the kernel's own ``c_load`` term then picks up the bias contribution.
    This costs one extra fused copy (M*N elements) per call but reuses the
    existing alpha-beta kernel + autotune cache without any kernel change.
    ...
    if bias is None:
        ...
        # NaN bits — which then leak through the beta=0 epilogue path and
        # produce all-NaN output non-deterministically.
        #
        # Zeroing ``out`` here costs ~one fused memset; on B200 BF16 GEMM
        # shapes this is well under 1% of total kernel time. The proper
        # long-term fix is a beta=0 specialization that skips the c_load
        # entirely (follow-up).
        out.zero_()
        beta_val = 0.0
    else:
        # Bias path: pre-broadcast bias into ``out`` and set beta=1 so the
        # kernel's ``c_load`` term contributes ``1 * bias_broadcast``. The
        # kernel signature and autotune config space are unchanged.
